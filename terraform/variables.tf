...
# ── Cloud Build CI/CD (opt-in; Coolify is default production host) ────────────
...
variable "enable_cloud_build_triggers" {
  description = "Grant Cloud Build SA deploy IAM and manage triggers. Keep false while Coolify hosts production."
  type        = bool
  default     = false
}
